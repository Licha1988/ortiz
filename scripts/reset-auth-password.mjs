...
let users;
try {
  // .env.local escapa cada "$" como "\$" para que dotenv no lo interprete
  // como expansión de variable; hay que desescaparlo antes de parsear JSON.
  const rawUsersJson = authUsersMatch[1].trim().replace(/\\\$/g, "$");
  users = JSON.parse(rawUsersJson);
} catch {
